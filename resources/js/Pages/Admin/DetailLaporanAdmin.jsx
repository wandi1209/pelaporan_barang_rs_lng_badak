...
export default function DetailLaporanAdmin({ auth, laporan, foto_laporan, asset, unit, ruangan }) {
    ...
    const { data, setData, put, post, delete: deleteRecord, errors, processing } = useForm({
        ...
        biaya_perbaikan: laporan.biaya_perbaikan,
    ...
    try {
        ...
        if (confirmed.isConfirmed) {
            ...
            // Assuming that the post request returns a promise
                deleteRecord(route('admin.laporan.foto.destroy', { id: fotoId })),
    ...
    try {
        ...
        if (confirmed.isConfirmed) {
            await deleteRecord(route('admin.laporan.destroy', { id: laporanId })),
            Inertia.visit(route('admin.laporan.index'));
        }
        } catch (error) {
        console.error('Error deleting record:', error);
        }
    ...
    const submitEdit = async (e) => {
        ...
        try{
           await Promise.all([
            // Assuming that the post request returns a promise
                put(route('admin.laporan.update', { id: laporan.id }),{
    ...
    const submitUpload = async (e) => {
        ...
        try{
            ...
            await Promise.all([
            // Assuming that the post request returns a promise
                post(route('admin.laporan.upload', { id: laporan.id }), formData, {
                    headers: {
                        'Content-Type': 'multipart/form-data',
                    },
                }),
    ...
    const submitUpdate = async (e) => {
        ...
        try{
           await Promise.all([
            // Assuming that the post request returns a promise
                put(route('admin.laporan.update.status', { id: laporan.id }),{
                    status: data.status,
            ...
            <div className="py-12">
                <div className="mx-auto sm:px-6 lg:px-8">
                    <div className="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                        ...
                        <div className='flex flex-wrap lg:flex-nowrap bg-gray-200 p-4 m-3 rounded-lg'>
                            ...
                            <div className='flex flex-wrap lg:flex-nowrap lg:w-2/3 text-gray-800 w-full rounded-lg pr-14 pb-5 lg:mx-12'>
                                <div className='pl-4 lg:w-1/2'>
                                    ...
                                    <button onClick={() => openModalUpdate()} type='button' className='flex items-center hidden lg:inline-flex justify-center mt-4 items-center text-white bg-sky-500 p-3 transition duration-300 hover:scale-110 hover:bg-sky-600 rounded-lg '>
                                        <Icon>
                                            update
                                        </Icon>
                                        <span className='ml-1'>
                                ...
                                <div className='pl-4'>
                                    ...
                                    <button onClick={() => openModalUpdate()} type='button' className='flex lg:hidden block justify-center mt-4 items-center text-white bg-sky-500 p-3 transition duration-300 hover:scale-110 hover:bg-sky-600 rounded-lg '>
                                        <Icon>
                                            update
                                        </Icon>
                                        <span className='ml-1'>
                        ...
                        <Modal show={isModalUpdate} onClose={closeModal}>
                            <form onSubmit={submitUpdate} encType="multipart/form-data">
                                ...
                                <div className='px-4 py-2 bg-gray-200'>
                                    {laporan.status != 'Selesai' || auth.user.role == 'admin' ? 
                                    <div className='mt-4'>
                                        ...
                                        >
                                            ...
                                            {laporan.status === 'Selesai' &&
                                                <option value='Menunggu Respon Petugas'>Menunggu Respon Petugas</option>
                                            }
                                    ...
                                    : null}
                                    {laporan.status === 'Sedang Diproses' || auth.user.role == 'admin' ?
                                    <div className='mt-4'>
                                        <InputLabel htmlFor="biaya_perbaikan" value="Biaya Perbaikan" />

                                        <TextInput
                                            id="biaya_perbaikan"
                                            type="text"
                                            name="biaya_perbaikan"
                                            value={data.biaya_perbaikan}
                                            className="mt-1 block w-full"
                                            autoComplete="current-biaya_perbaikan"
                                            onChange={(e) => setData('biaya_perbaikan', e.target.value)}
                                        />

                                        <InputError message={errors.biaya_perbaikan} className="mt-2" />
                                    </div> : null}
                                    <div className='mt-4'>
                                        <InputLabel htmlFor="catatan_petugas" value="Catatan Petugas" />
                                        ...
                                        <TextArea
                                            ...
                                            className="mt-1 block w-full"
                                            autoComplete="current-catatan_petugas"
                                            onChange={(e) => setData('catatan_petugas', e.target.value)}
                                        >{data.catatan_petugas}</TextArea>

                                        <InputError message={errors.catatan_petugas} className="mt-2" />
                                    </div>
                                </div>
                                <div className='p-3 bg-gray-50'>
                                    <PrimaryButton className='ml-2' disabled={processing} >
