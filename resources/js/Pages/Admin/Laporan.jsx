...
export default function Laporan({ auth, laporan }) {
  ...
  const filterSubmit = async (e) => {
    e.preventDefault();
    await Inertia.visit(route('admin.laporan.index', {
      startDate,
      endDate,
  }));
  }
  ...
  const columns = [
    ...
    { Header: 'Aksi', accessor: 'actions', disableGlobalFilter: true, disableSortBy: true, Cell: (row) => (
        <div className='text-white flex justify-center items-center'>
          ...
          <Link href={route('admin.laporan.show', row.row.original.id)}>
  ...
  return (
    <AuthenticatedLayout
      user={auth.user}
      header={<h2 className="font-semibold text-xl leading-tight">Laporan</h2>}
    >
      <Head title="Laporan" />
